(function () {
  ...
  // here can't halt the rest of the boot sequence.
  try { (function wireSpotlightExpired() {
    const btn = $("#spotlight-expired-btn");
    const menu = $("#spotlight-expired-menu");
    if (!btn || !menu) return;
    const SPOTLIGHT_ID = "32/2024/QH15";

    function findExpiredRelated() {
      const sp = H.findDoc(SPOTLIGHT_ID);
      if (!sp) return [];
      const docs = Object.values(DB);
      const ids = new Set();
      const docReplaces = Array.isArray(sp.replaces) ? sp.replaces : [];
      for (const id of docReplaces) ids.add(id);
      for (const o of docs) {
        if (o.id === sp.id) continue;
        const oReplaces = Array.isArray(o.replaces) ? o.replaces : [];
        if (oReplaces.includes(sp.id) || (o.status && o.status.includes(sp.id))) ids.add(o.id);
        if (sp.status && sp.status.includes(o.id)) ids.add(o.id);
      }
      // Also pick up anything cited from the spotlight body that's already expired.
      // collectAllRefsInDoc depends on regex consts that are in the TDZ at boot,
      // so guard with try/catch — at boot we just skip it; on user click it's fine.
      try {
        const refs = collectAllRefsInDoc(sp);
        for (const r of refs) if (r.docId) ids.add(r.docId);
      } catch { /* boot-time TDZ: skip body refs, still get replaces relations */ }
      const list = [];
      const seen = new Set();
      for (const id of ids) {
      ...
      list.sort((a, b) => (b.expiryDate || "").localeCompare(a.expiryDate || ""));
      return list;
    }

    const countBadge = $("#spotlight-expired-count");
    function renderMenu() {
      const items = findExpiredRelated();
      if (countBadge) {
        if (items.length) {
      ...
      if (!items.length) {
        menu.innerHTML = `<li class="se-empty">Không có văn bản liên quan hết hiệu lực.</li>`;
        return;
      }
      menu.innerHTML = items.map(d => `
        <li role="menuitem" data-doc-id="${escapeHtml(d.id)}">
          <span class="se-type ${d.typeKey || ''}">${escapeHtml(d.type)}</span>
          <span class="se-body">
            <span class="se-title">${escapeHtml(d.shortTitle)}</span>
            <span class="se-meta">${escapeHtml(d.number)} · Hết hiệu lực ${formatDate(d.expiryDate || "")}</span>
          </span>
        </li>
      `).join("");
      $$("li[data-doc-id]", menu).forEach(li => {
        li.addEventListener("click", () => {
          openDoc(li.dataset.docId);
          closeMenu();
        });
      });
    }
    ...
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".spotlight-expired-wrap")) closeMenu();
    });
    // initial badge population (without opening)
    renderMenu();
    closeMenu();
  })(); } catch (err) { console.error("wireSpotlightExpired failed:", err); }
